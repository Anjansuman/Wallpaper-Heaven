...
export default function PhoneFilterTab() {
  ...
  return (
    <div className="lg:hidden border border-[#0B2814] rounded-md px-2 py-1 flex justify-between items-center">
      <div
        className="w-full flex justify-between items-center"
        onClick={() => setFilterTab((v) => !v)}
      >
        <div>Filter</div>
        <IconFilter className="w-4 h-4 md:w-5 md:h-5" />
      </div>

      {showFilter && (
        <div
          ref={filterRef}
          className="md:hidden h-screen w-full sm:w-[50%] fixed z-50 top-0 right-0"
        >
...
const PhoneFilterElements = ({ close }: PhoneFilterElementsProps) => {
  ...
  const toggleOption = (option: string) => {
    ...
    // Update URL
    const query = updated.length ? `?filter=${updated.join("+")}` : "";
    router.replace(`${location.pathname}${query}`);
  };

  return (
    <div className="h-screen w-full bg-[#faf0e6] py-3 md:py-5 px-6 flex flex-col justify-between gap-6">
      <div>
        ...
        <div className="w-full flex flex-col pt-7 p-4">
            ...
            {options.map((option, index) => (
              <label key={index} className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  id={`checkbox-${index}`}
                />
                <div className={`w-4 h-4 border-2 border-[#0B2814] rounded-xs ${selected.includes(option) ? "bg-[#0B2814]" : ""}`}></div>
