...
export default function Brands() {
  ...
  return (
    <div className="relative w-full flex flex-col gap-y-4 ">
      ...
      <button
        onClick={() => scroll("left")}
        className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 dark:bg-black/50 backdrop-blur-md p-2 rounded-full shadow-md"
      >
      ...
      <div
        ref={scrollRef}
        className="w-full flex gap-x-4 overflow-x-scroll [::-webkit-scrollbar]:hidden [scrollbar-width:none] scroll-smooth "
      >
        {[...Array(10)].map((_, i) => (
      ...
      <button
        ...
        className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 dark:bg-black/50 backdrop-blur-md p-2 rounded-full shadow-md"
      >
        <ChevronRight className="w-6 h-6 text-black dark:text-white" />
      </button>
        <div className="h-28 w-full relative">
            <Image src={tree} alt="tree" className="w-full h-full object-cover object-[center_70%] top-10 rounded-lg " />
            <div className="absolute top-1/2 left-1/2 -translate-1/2 text-4xl font-extrabold  ">
